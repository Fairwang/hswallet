    ...
    for case in case_list:
        ...
        if method.upper() == 'GET':
            ...
            if param=='':
                ...
                if case_id==1:
                    code_msg7=str(code_msg())
                    results = requests.post(url, headers=headers, json=code_msg7[0:4]).text
                else:
...
def copy_excel(file_path, res_flags, request_urls, responses):
    ...
    name=time.strftime('%Y%m%d%H%M%S')+"wallet_check_pay_passwd_code.xls"
    filename =os.path.dirname(os.path.dirname(os.path.dirname(__file__))) + "/report/" + name
...
if __name__ == '__main__':
    try:
        # filename = sys.argv[1]
        filename=os.path.dirname(os.path.dirname(os.path.dirname(__file__)))+"/cases/alone/"+"wallet_check_pay_passwd_code.xls"
        print filename
    except IndexError, e:
